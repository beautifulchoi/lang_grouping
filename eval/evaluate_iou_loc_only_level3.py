...
import sys
sys.path.append("/root/shan/lang_grouping")
import colormaps
...
def run(cfg: DictConfig) -> None:
    ...
    ae_ckpt_path = os.path.join(cfg.ae_ckpt_dir, dataset_name, "best_ckpt.pth")
...
def lerf_localization(sem_map, image, clip_model, image_name, img_ann):
    ...
    for k in range(len(positives)):
        ...
        torch_relev = 0.5 * (avg_filtered + select_output.unsqueeze(-1))[0] # TODO error debugging
...
def evaluate(feat_dir, output_path, ae_ckpt_path, json_folder, mask_thresh, encoder_hidden_dims, decoder_hidden_dims, logger):
    ...
    compressed_sem_feats = np.zeros((1, len(eval_index_list), *image_shape, 3), dtype=np.float32)
    # (feature level 1,2,3, index image shape, feat dim)

    for i in range(len(feat_dir)):
        if feat_dir[i].endswith("3"):
            feat_paths_lvl = sorted(glob.glob(os.path.join(feat_dir[i], '*.npy')),
                                key=lambda file_name: int(os.path.basename(file_name).split(".npy")[0]))
            for j, idx in enumerate(eval_index_list):
                compressed_sem_feats[0][j] = np.load(feat_paths_lvl[idx]) #TODO error : compressed_sem_feats 731 988 // rendered feature : 731 989
    ...
    for j, idx in enumerate(tqdm(eval_index_list)):
        ...
        sem_feat = compressed_sem_feats[:, j, ...] # [2:]
        sem_feat = torch.from_numpy(sem_feat).float().to(device)
